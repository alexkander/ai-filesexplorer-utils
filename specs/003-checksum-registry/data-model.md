...
| `resolvedByPass2`      | `boolean`                                     | Added post-implementation. `true` once Pass 2 has concluded _something_ for this pair at least once (`Matching` or `Differs`) — the missing signal that `directoryChecksum === null` alone can't provide, since that value is identical for "confirmed Differs" and "Pass 2 never actually reached this pair" (never compared, or cancelled by Stop before concluding). Sticky across Pass 1 relistings (FR-016); only reset to `false` by a `mode: 'full'` re-compare or on first row creation. |
