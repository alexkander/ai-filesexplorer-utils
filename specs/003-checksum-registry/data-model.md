...
| `hasUnreadableEntries` | `boolean`                                     | `true` if any direct child couldn't be listed, or a direct file's content couldn't be read when Pass 2 tried to checksum it (FR-011).                                                                                                                                                                              |
...
| `status` | `'not_compared' \| 'matching' \| 'differs' \| 'only_left' \| 'only_right' \| 'scanning' \| 'error'` | `only_left`/`only_right` if no counterpart row exists on the other side. `scanning` if Pass 1 or Pass 2 currently has this path (or an ancestor) as its active unit of work. `error` if `hasUnreadableEntries` applies to this path or an ancestor on either side (FR-011a). Otherwise: for a file, compare `fullChecksum` (or the earliest cascade stage that already proved a difference) between sides; for a directory, `matching` iff its `directoryChecksum` is set and equal on both sides, `differs` otherwise once Pass 2 has resolved it, else `not_compared`. |
